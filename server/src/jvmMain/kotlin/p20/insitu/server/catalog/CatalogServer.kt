...
import p20.insitu.com.catalogApi.CatalogApiSettings

val keycloakProvider = OAuthServerSettings.OAuth2ServerSettings(
    name = "keycloak",
    authorizeUrl = AuthApiSettings.AUTH_ENDPOINT,
    accessTokenUrl = AuthApiSettings.TOKEN_ENDPOINT,
    clientId = CatalogApiSettings.CATALOG_CLIENT_ID,
    clientSecret = CatalogApiSettings.CATALOG_CLIENT_SECRET,
    accessTokenRequiresBasicAuth = false,
    requestMethod = HttpMethod.Post, // must POST to token endpoint
    defaultScopes = listOf("openid roles")
)

fun startCatalogServer() {
    embeddedServer(CIO, host =  CatalogApiSettings.BASE_CATALOG_URL, port = CatalogApiSettings.CATALOG_PORT) {
